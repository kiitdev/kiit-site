...
// ---- write src/examples/<module>/, starting clean so removed examples don't leave stale files behind
if (errors.length) {
  warnings.forEach((w) => console.warn(`warning: ${w}`));
  errors.forEach((e) => console.error(`error: ${e}`));
  process.exit(1);
}
const outDir = resolve(site, 'src/examples', map.module.name);
...
for (const entry of Object.values(out)) {
  for (const item of entry.items) {
    for (const lang of Object.keys(item.snippets)) {
      const list = item.snippets[lang];
      list.forEach((snippet, n) => {
        const file = `${item.id}/${list.length > 1 ? `${lang}-${n + 1}` : lang}.text`;
        ...
        if (!fileKeys.includes(file)) {
          fileKeys.push(file);
          imports.push(`import f${fileKeys.length - 1} from './${file}';`);
        }
      });
    }
  }
...
writeFileSync(resolve(outDir, 'examples.json'), JSON.stringify({examples: out}, null, 2) + '\n');
writeFileSync(
  resolve(outDir, 'files.ts'),
  `// Generated by scripts/extract-examples.mjs, do not edit.\n${imports.join('\n')}\n\n` +
    `const files: Record<string, string> = {\n${fileKeys.map((k, i) => `  '${k}': f${i},`).join('\n')}\n};\n\nexport default files;\n`,
);

console.log(`Wrote ${relative(site, outDir)}: ${Object.keys(out).length} topics, ${usedIds.size} examples mapped, ${fileCount} code files`);
